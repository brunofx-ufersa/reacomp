...
Julia possui um ambiente REPL (_read-eval-print loop_) para executar pequenas instruções diretamente no console, sem a necessidade de salvar o código em um arquivo e compilar, por exemplo. Para testar essa funcionalidade digite `julia` em sem terminal. 
...
Abra o explorador, o primeiro botão na barra vertical esquerda com ícone de duas folhas sobrepostas ou digite `Ctrt+Shift+E`. É nesse painel que você verá todas as suas pastas de arquivos. 

Clique em `Open Fold` e selecione uma pasta recem criada, por exemplo `Tutorial Julia`.
...
Para nosso código, qualquer opção é válida, pois contém somente uma linha. Ao realizar um desses comandos pela primeira vez, uma janela de terminal será aberta com o REPL de Julia. Se você ver a mensagem "Hello, World!", parabéns.
