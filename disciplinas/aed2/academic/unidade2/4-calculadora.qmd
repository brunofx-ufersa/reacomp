...
- **Modularização da UI**: Mova as funções `select_operation_int` e `select_operation_double` para um novo par de arquivos de utilitário (por exemplo, `ui.c`/`ui.h`). Isso ajuda a manter o arquivo principal (`calculator.c`) focado apenas na lógica de cálculo, separando a **Interface do Usuário** da **Lógica de Negócios**.
